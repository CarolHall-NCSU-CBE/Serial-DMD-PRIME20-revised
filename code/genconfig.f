subroutine genconfig
	...
	logical exist_flag1,exist_flag2
      ...
      !iflag = 197071101   !1331171207 !1058472402
	iflag =time()
	write(6,*) 'Random seed for initial configuration: ', iflag
      xr=drandm(iflag)
      call srand(iflag)
	boxl = boxlength
	reducedtemp=0.50
...
9993		xtmp(nop1+(i-1)*numbeads2+1)=(drandm(0))*boxl
       ...
       enddo
        do k=(i-1)*numbeads2+1,i*numbeads2
	      		do l=1, nop1+(i-1)*numbeads2
                       		rxij=xtmp(nop1+k)-xtmp(l)
                       		ryij=ytmp(nop1+k)-ytmp(l) 
                       		rzij=ztmp(nop1+k)-ztmp(l)
                      	 	rxij=rxij-boxl*aint((rxij/boxl)+.5)
                       		ryij=ryij-boxl*aint((ryij/boxl)+.5)
                       		rzij=rzij-boxl*aint((rzij/boxl)+.5)
	               		rijsq=rxij*rxij+ryij*ryij+rzij*rzij
