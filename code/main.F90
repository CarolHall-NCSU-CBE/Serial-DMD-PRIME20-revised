...
#include "header.f"
	...
	integer totalsteps, updatedcd
      	...
      	real*8  ehh_ii,ehh_ij,shldrdisi,shldrdisj
	integer kstart,kend, ratingi, ratingj, hb_partneri, hb_partnerj 
	integer ncim1wr, ncaiwr, ncajwr, nnjp1wr,xtrp
...
!                    		j is c, hb_partner is n
		...
		elseif (evcode .eq. 45) then
				hb_partneri = extra_repuls(i,4)
                  		if (identity(i) < identity(hb_partneri)) then
!                    		i is n, hb_partner is c
                     			call repuls_check_3(i,hb_partneri,j,rating)
                  		else
!                    		i is c, hb_partner is n
                     		call repuls_check_3(hb_partneri,i,j,rating)
                  		endif
				ratingi = rating

				hb_partnerj = extra_repuls(j,4)
                  		if (identity(j) < identity(hb_partnerj)) then
!                    		j is n, hb_partner is c
                     		call repuls_check_3(j,hb_partnerj,i,rating)
                  		else
!                    		j is c, hb_partner is n
                     		call repuls_check_3(hb_partnerj,j,i,rating)
                  		endif
		  		ratingj = rating
				
                  		if ((ratingi .gt. 10.d0).and.(ratingj .gt. 10.d0)) then
		     			coltype(i) = 15
 	          		else
 	             			coltype(i) = 5
                  		endif
...
!              	must decide if atoms are moving toward each other via square square shoulder (=6) or non-event (=15) 
		...
		elseif (evcode .eq. 45) then
				hb_partneri = extra_repuls(i,4)
				hb_partnerj = extra_repuls(j,4)
		  		if ((bptnr(i) .ne. hb_partneri).and.(bptnr(j) .ne. hb_partnerj)) then
                     			coltype(i) = 15
				else
					call check_sigma(i,hb_partneri,rating)
					ratingi = rating
					call check_sigma(j,hb_partnerj,rating)
					ratingj = rating
					if ((ratingi .le. 10.d0).or.(ratingj.le. 10.d0)) then
                        			coltype(i) = 13
	             			else
		        			coltype(i) = 6
		     			endif
...
!                       				i is n, j is c
							...
							if ((extra_repuls(i,3).ne.0).and.(extra_repuls(j,3).ne.0)) then
								if ((extra_repuls(i,3).eq.extra_repuls(i,2)).and.(extra_repuls(j,3).eq.extra_repuls(j,2))) then
									ev_code(extra_repuls(i,3),i) = xrepuls1
									ev_code(i,extra_repuls(i,3)) = xrepuls2
									ev_code(extra_repuls(j,3),j) = xrepuls1
									ev_code(j,extra_repuls(j,3)) = xrepuls2
								endif
							endif
		...
		elseif (evcode .eq. 45) then
				...
				if (ratingi.lt.ratingj) then
					hb_partner = extra_repuls(i,4)
		                 ...
		                 elseif (ratingi .gt. ratingj) then
					hb_partner = extra_repuls(j,4)
                  				if (identity(j) + identity(hb_partner) .eq. 5) then
                     					bptnr(j)=hb_partner
                     					bptnr(hb_partner)=j
			...
			elseif (evcode .eq. 45) then
				if (ratingi.gt.ratingj) then
					hb_partner = extra_repuls(i,4)
		                ...
		                elseif (ratingi .lt. ratingj) then
					hb_partner = extra_repuls(j,4)
                  			if (identity(j) .ge. 5) then
                     				bptnr(j)=0
                     				bptnr(hb_partner)=0
				...
				elseif ((ratingi.eq.11).and.(ratingj.eq.11)) then
					!write(217,'(8i26)') coll, i, bptnr(i), identity(i), j, bptnr(j),identity(j),coltype(i)
					hb_partner = extra_repuls(i,4)
					if (identity(i) .ge. 5) then
                     				bptnr(i)=0
                     				bptnr(hb_partner)=0
