...
!VN: get paths for file openning and creating:
	...
	if (annealcheck == 0) then
		annealingsteps = 9
		allocate(anneal(annealingsteps))
		anneal= (/0.50,0.45,0.40,0.35,0.30,0.28,0.26,0.24,0.22/)	
	...
	elseif (annealcheck == 1) then
		...
		do j = 1, annealingsteps
			anneal(j) = ((maxannealing -(j-1)*incrannealing)+115.79)/2288.46
		enddo
		allocate(collset(1))
		collset = annealcoll
	endif
...
8       format(3F12.4)	
		...
		if (arg(1) .eq. 'traj') then
			...
			allocate(xtmp(noptotal),ytmp(noptotal),ztmp(noptotal))
			call chdir(rundir)
			izero = ichar('0')
			read(initfile,*) start
			read(endfile,*) end
! 	3. Initialize it with the names of xtc files you want to read in and write out
    			!call xtc_out % init('analysis/run'//initfile//'to'//endfile//'.xtc', 'w')
			!call xtc_out % read
			!xtc_out % STEP = 0
			!xtc_out % prec = 1000.00
			!xtc_out % NATOMS = noptotal + chnln1*nc + chnln2*nc2
			!allocate(xtcf % pos(3, xtcf % NATOMS))
			!xtc_out % box = reshape((/boxl,0.0,0.0,0.0,boxl,0.0,0.0,0.0,boxl/), (/3,3/))
			do i = start, end
				...
				do while (.true.)
					read(runcf) coll,t,xtmp, ytmp, ztmp
					do k=1,(noptotal)
						sv(1,k) = xtmp(k)*boxl
						sv(2,k) = ytmp(k)*boxl
						sv(3,k) = ztmp(k)*boxl
      					enddo
					write(*,*) fname_digits, coll, t
					do k = 1, 10
						write(*,*) sv(1,k), sv(2,k), sv(3,k)
					enddo

					!xtc_out % time = ttotal+t
					!xtc_out % STEP = xtc_out % STEP + 1 
					!xtc_out % pos = sv(1:3,:)
		
					! Just an example to show what was read in
        				!write(*,'(a,f12.6,a,i0)') " Time (micros): ",xtc_out % time, "  Step: ", xtc_out % STEP
        				!write(*,'(a,f12.6,a,i0)') " Precision: ", xtc_out % prec, "  No. Atoms: ", xtc_out % NATOMS
				
        				!write(*,'(3f9.3)') xtc_out % pos
					...
					if((coll.gt.0).and.(mod((coll-1),10000000).eq.0)) goto 114
				enddo
114				close(runcf)
				ttotal=ttotal+t
...
!!!!!!! Energy and temperature vs time:
		elseif (arg(1) .eq. 'evst') then 
			...
			do i = start, end
				...
				if (i.gt.1) then
					read(rune,'(i15,3f12.4,3i8,4f12.4)') coll,t,ered,tred,hb_alpha,hb_ii,hb_ij,ehh_ii,ehh_ij,rg_avg,e2e_avg
				endif
				if (i.le.annealingsteps) then
					timetemp = anneal(i)
				else
					timetemp = simtemp
				endif
				do while (.true.)
					...
					write(3878,'(f15.4,f24.4,f18.1,f20.4,2f14.4)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(timetemp*12),realtemp,ered,e_kin,(ered-0.5*tred*3.0*noptotal)
...
!!!!!!! Hydrogen bonding vs time: 
		elseif (arg(1) .eq. 'hbvst') then
			...
			do i = start, end
				...
				do while (.true.)
					read(rune,'(i15,3f12.4,3i8,4f12.4)') coll,t,ered,tred,hb_alpha,hb_ii,hb_ij,ehh_ii,ehh_ij,rg_avg,e2e_avg
					write(428,'(f15.4,f24.4,3i15)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(timetemp*12),(hb_ii+hb_ij),hb_ij,hb_alpha
...
!!!!!!! Sidechain-sidechain interaction energy vs time: 
		elseif (arg(1) .eq. 'scvst') then
			...
			do i = start, end
				...
				do while (.true.)
					read(rune,'(i15,3f12.4,3i8,4f12.4)') coll,t,ered,tred,hb_alpha,hb_ii,hb_ij,ehh_ii,ehh_ij,rg_avg,e2e_avg
					write(72,'(f15.4,2f24.4,f30.4)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(tred),ehh_ii,ehh_ij
		...
		elseif (arg(1) .eq. 'scvst') then
			...
			do i = start, end
				...
				do while (.true.)
					...
					write(72,'(f15.4,2f24.4,f30.4)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(tred),ehh_ii*12.47,ehh_ij*12.47
		...
		elseif (arg(1) .eq. 'cluster') then
			...
			do i = start, end
				...
				do while (.true.)
					...
					call readparameters
					call clustertrack
					write(258,'(f15.4,f24.4,i14,2i10)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(tred),coil,oligomer,fibril
		...
		elseif (arg(1) .eq. 'aggregation') then
			...
			do i = start, end
				...
				do while (.true.)
					...
					write(3878,'(f15.4,f24.4,f18.1,f20.4,2f14.4)') (colltotal+real(coll)/1000000000.0d0),(ttotal+t)*0.96*0.001*3.3/sqrt(tred),realtemp,ered*12.47,tred*12.47,(ered-0.5*tred*3.0*noptotal)*12.47
