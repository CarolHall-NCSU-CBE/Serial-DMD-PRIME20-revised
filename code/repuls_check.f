...
#include "def.h"
	...
	implicit none
  	...
  	real*8 vxij,vyij,vzij,rxij,ryij,rzij,rating
    	real*8 d1,d2,d3,d4
	integer i,j
	!open(924,file='rating.txt',status='unknown',position='append')
     	if (i .le. nop1) then
     		ncim1=i+chnln1-1
           	ncai=i-chnln1
      	else
	...
	if ((d1 > shder_dist1) .and. (d2 > shder_dist2) .and. (d3 > shder_dist3) .and. (d4 > shder_dist4)) then
   		rating = 10.d0
		if (bptnr(nnjp1).eq.ncim1) then
			rating = 12.d0
		endif
     	else
  		rating = 11.d0
		!write(924,'(i17,6i6,4f11.4)') coll, i, j, ncaj, nnjp1, ncai, ncim1, (d1-shder_dist1), (d2-shder_dist2), (d3-shder_dist3), (d4-shder_dist4)
    	endif
	!close(924)
   	return
